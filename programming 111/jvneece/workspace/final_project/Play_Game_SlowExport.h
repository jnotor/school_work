...
// Class Description: 
	...
	//tests are using hard set values to speed up test times; remove those later
#pragma once
...
#ifndef PLAY_GAME_SLOWEXPORT_H
#define PLAY_GAME_SLOWEXPORT_H

class Play_Game_SlowExport {
public:
	Play_Game_SlowExport();
	void playGame();
	void initializeBets();
...
private:
	...
	bool validResponse, flag, flag2, aceFlag, dealerAceFlag, surrenderFlag;
...
Play_Game_SlowExport::Play_Game_SlowExport() {
	funds = 200;
	holdFunds = 0;
	playerBet = 0;
	insuranceBet = 0;
...
void Play_Game_SlowExport::playGame() {
	...
	while (funds > 0) {
		...
		//cin >> yes_or_no;
		yes_or_no = 'y';														//for speed testing purposes
		if (yes_or_no == 'y') {
			gameDeck.setDeck();
			round = 4;  
			initializeBets();
...
void Play_Game_SlowExport::initializeBets() {
...
void Play_Game_SlowExport::initializeHands() {
	cout << "Your starting hand is: " << endl;
	firstCard =11;//gameDeck.getNextCard(0, user.getScore());					//test
	aceFlag = gameDeck.aceChecker(firstCard);									
	cout << "and ";
	secondCard = 10;//gameDeck.getNextCard(1, user.getScore());					//test
...
void Play_Game_SlowExport::dealerSecond() {
	dSecondCard = rand() % 11;
	if (dSecondCard == 1) {
		dSecondCard = 11;
	}
...
void Play_Game_SlowExport::displayScores(int userScore, int dealerScore) {
	cout << endl << "Your Score: " << userScore << endl;
	cout << "Known Dealer Score: " << dealerScore << endl;
}

void Play_Game_SlowExport::gameMenu() {
	validResponse = false;
	while (!validResponse) {
		displayScores(user.getScore(), dealer.getScore());
...
void Play_Game_SlowExport::dealerTurn() {
	if (flag) {
		dealer.addScore(dSecondCard);
...
void Play_Game_SlowExport::results() { //need to figure out the sequence it should go in to rule out dealer busts
	if (dealer.getScore() == user.getScore()) {
		cout << "Push! " << endl;
		playerBet = 0 - playerBet;
		user.placeBet(playerBet);
	}
	else if (((dealer.getScore() > 21) || (dealer.getScore() < user.getScore())) && (flag)) { //flag being true means dealer took his turn
		cout << "You Win! " << endl;
		playerBet = (0 - playerBet) * 2;
		user.placeBet(playerBet);
		cout << "User chips total: " << user.getBet() << endl;
	...
	else if ((dealer.getScore() == 21) || (dealer.getScore() > user.getScore()) || (surrenderFlag)) {
		cout << "You Lose! " << endl;
		cout << "User chips left currently: " << user.getBet() << endl;
	}
...
void Play_Game_SlowExport::postRoundInfo() {
	funds = user.getBet();
	holdFunds = funds;
	if (yes_or_no =~ 'y') {
		funds = 0;
...
void Play_Game_SlowExport::resets() {
	if (!flag2) {
		funds = user.getBet();
	}
	holdFunds = 0;
