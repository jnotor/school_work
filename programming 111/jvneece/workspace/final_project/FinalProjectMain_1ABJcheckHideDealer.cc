...
void gameMenu(Player user, Player dealer, Deck gameDeck) {			// Definition of gameMenu Function
	...
	if (yes_or_no == 'y') {
		...
		firstCard = showNextCard(gameDeck.getNextValue(0, user.getScore()));
		cout << "and ";
		secondCard = showNextCard(gameDeck.getNextValue(1, user.getScore()));		//this won't work
		...
		cout << "The dealer started with ";
		dfirstCard = showNextCard(gameDeck.getNextValue(2, dealer.getScore()));
		cout << "showing." << endl;
		dealer.addScore(dfirstCard);
		// if (dfirstCard + gameDeck.getNextCard(3, dealer.getScore()) == 21)   {	//currently reports dealer's hidden card
		// 	BJcount += 10;
		// }
		...
		while (flag) {
			...
			if (flag) {
				...
				dealer.addScore(gameDeck.getNextCard(3, dealer.getScore())); //reveals hidden card
